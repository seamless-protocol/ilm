...
methods {
    ...
    // Ignore the concrete function implementation
...
// rebalance() cannot change the strategy equity when slippage is zero
...
// A witness - equity may change when slippage is nonzero
...
// A witness - equity may change when slippage is nonzero - calling rebalanceDown()
...
// A witness - equity may change when slippage is nonzero - calling rebalanceUp()
...
// Equity per share cannot decrease after redeem() if rebalance is not needed
rule equity_per_share_non_decreasing {
    ...
    uint256 assets_redeemed = redeem(e2, shares_to_redeem, receiver, owner, minUnderlyingAsset);

    uint256 equityUSD_after = equityUSD();
    mathint totalSupply_after = totalSupply_before - shares_to_redeem;

    require totalSupply_before == 150; //under-approximation
    require shares_to_redeem == 50;
    assert  equityUSD_after * totalSupply_before >=  equityUSD_before *  totalSupply_after;
}

// A witness equity per share may decrease if rebalance is needed
...
//  rebalance() changes the collateral ratio in the desired direction (upwards)
rule rebalance_direction_non_decreasing
{
    env e1;
...
//  rebalance() changes the collateral ratio in the desired direction (downwards)
...
// rationMargin is not greater than 1 USD
