...
contract LoopStrategyScenariosTest is LoopStrategyTest {
  ...
  function _setupScenario(
    ...
    uint256 targetRatio = USDWadRayMath.usdDiv(targetLeverage, targetLeverage - 100);
    uint256 rebalanceDiff = PercentageMath.percentMul(targetRatio, 3_00);
    ...
    _changePrice(CbETH, 1798_87843722);
    ...
    uint256 startDebtUSD = (startCollateral * 1798_87843722 * (startLeverage - 100)) / (startLeverage * 1e18); 
    uint256 startDebt = startDebtUSD / 1e2;

    vm.startPrank(address(strategy));
      deal(address(CbETH), address(strategy), startCollateral);
      lendingPool.pool.supply(address(CbETH), startCollateral, address(strategy), 0);
      lendingPool.pool.borrow(address(USDbC), startDebt, lendingPool.interestRateMode, 0, address(strategy));
  ...
  function test_scenario_1_depositWhenPoolAtTarget() public {
    ...
    assertEq(strategy.currentCollateralRatio(), 150000000);
    ...
    assertApproxEqAbs(strategy.currentCollateralRatio(), 150000000, USD_DELTA);
  ...
  function test_scenario_2_depositWhenPoolBelowMinForRebalance() public {
    ...
    assertEq(strategy.currentCollateralRatio(), 133333333);
    ...
    assertApproxEqAbs(strategy.currentCollateralRatio(), 140000000, USD_DELTA);
  ...
  function test_scenario_5_rebalanceWhenCollateralPriceChange() public {
    _setupScenario(100 ether, 300, 300, 1000 ether);
    _changeLtv(CbETH, 90_00, 95_00, 105_00);
