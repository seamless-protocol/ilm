...
contract DeployILMRegistry is Script, BaseMainnetConstants {
    ...
    address internal constant ETHUSDCLOOP =
    ...
    function run() public {
        ...
        assert(
            registry.hasRole(
                registry.MANAGER_ROLE(), SEAMLESS_COMMUNITY_MULTISIG
            )
        );
        ...
        assert(!registry.hasRole(registry.DEFAULT_ADMIN_ROLE(), deployer));
        assert(!registry.hasRole(registry.MANAGER_ROLE(), deployer));
